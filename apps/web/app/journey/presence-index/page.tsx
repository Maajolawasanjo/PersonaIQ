...
import { Target, Clock, ArrowLeft, ArrowRight, CheckCircle2, Sparkles, Zap } from 'lucide-react';
...
export default function PresenceIndexPage() {
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-8 animate-fadeIn py-4">
      {/* 1. PRESENCE INDEX™ Dynamic Hero Card */}
      <div className="bg-white border border-gray-200 rounded-[24px] p-8 shadow-xs text-center space-y-6">
        <span className="text-[11px] font-mono font-bold text-gray-500 tracking-wider uppercase block">
          PRESENCE INDEX™ • REAL-TIME TELEMETRY
        </span>

        {/* Circular Glowing Ring Gauge */}
        <div className="inline-flex flex-col items-center justify-center relative my-2">
          <div className="w-44 h-44 rounded-full border-[10px] border-red-100 border-t-red-600 border-r-red-600 flex flex-col items-center justify-center bg-white shadow-lg relative">
            <div className="absolute inset-0 rounded-full border-4 border-red-500/40 animate-pulse pointer-events-none" />
            ...
            <span className="text-[11px] font-mono font-bold text-red-600 tracking-wider uppercase mt-1">
              {score >= 95 ? 'EXCELLENT' : score >= 88 ? 'HIGH AUTHORITY' : 'OPTIMAL'}
            </span>
          </div>
        </div>

        <p className="text-[13.5px] text-gray-600 font-medium max-w-md mx-auto leading-relaxed">
          Your synthesized presence index for <strong className="text-gray-950 uppercase">{occasion}</strong> is active. Calculated from skin undertone, attire contrast, and grooming telemetry.
        </p>
      </div>
      ...
      <div className="space-y-4 pt-2">
        <div className="flex items-center justify-between border-b border-gray-100 pb-3">
          <h2 className="text-[24px] font-bold text-gray-950 font-sans">
            Presence Action Protocols
          </h2>
          <span className="text-[11px] font-mono font-bold text-red-600 bg-red-50 px-3 py-1 rounded-full border border-red-200 uppercase">
            2 DYNAMIC ACTIONS
          </span>
        </div>
        ...
        <div className="bg-white border border-gray-200 rounded-[20px] p-6 shadow-xs relative overflow-hidden space-y-4 border-l-4 border-l-red-600">
          <div className="flex items-start justify-between">
            <div className="flex items-center space-x-2.5">
              ...
              <span className="text-[11px] font-mono text-gray-500 flex items-center space-x-1">
                ...
                <span>96% AI Alignment</span>
              </span>
            </div>
            <span className="text-[11px] font-mono text-gray-500 flex items-center space-x-1">
              <Clock className="w-3.5 h-3.5 text-gray-400" />
              <span>TIME REQ ~10 mins</span>
            </span>
          </div>

          <div className="space-y-2">
            <h3 className="text-[20px] font-bold text-gray-950 font-sans">
              Press your {outfitTitle} before your {occasion} engagement
            </h3>
            ...
            <div className="bg-gray-50 border border-gray-150 rounded-[14px] p-4 space-y-1">
              ...
              <p className="text-[12.5px] text-gray-600 font-normal leading-relaxed">
                Crisp lapel lines and uncreased shoulders communicate precision authority and elevated attention to detail during high-stakes presentations.
              </p>
            </div>
          </div>
          ...
          <div className="flex justify-end pt-1">
            <button
              ...
              className={`h-9 px-5 text-[12.5px] font-bold rounded-[8px] border transition-colors cursor-pointer ${
                doneActions.includes(1)
                  ? 'bg-emerald-600 text-white border-emerald-600'
                  : 'bg-white text-gray-700 border-gray-300 hover:bg-gray-50'
              }`}
            >
              {doneActions.includes(1) ? '✓ Protocol Completed' : 'Mark Protocol Complete'}
            </button>
          </div>
        </div>

        {/* Card 2: Polish footwear */}
        <div className="bg-white border border-gray-200 rounded-[20px] p-6 shadow-xs space-y-3">
          ...
          <h3 className="text-[18px] font-bold text-gray-950 font-sans">
            Ensure footwear leather is polished and matched to watch belt tone
          </h3>
        </div>
      </div>
      ...
      <div className="pt-4 flex items-center justify-between">
        ...
        >
          ...
          <span>Back to Fitting Room</span>
        </Link>
        <Link
        ...
        >
          <span>View Final Presence Plan™</span>
